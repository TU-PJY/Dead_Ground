...
glm::mat4 result, view, projection, s_mat, r_mat, t_mat, cam_mat;
GLfloat alpha;
...
void fix_image_at(GLfloat x, GLfloat y) {
    ...
    t_mat = translate(t_mat, vec3(cam_x, cam_y, 0.0));
    t_mat = rotate(t_mat, radians(-cam_rotation), vec3(0.0, 0.0, 1.0));
    t_mat = translate(t_mat, vec3(x, y, 0.0));
}
